...
public class ActiveBotAnalysis {
	...
	private static String[] families = { "dirtjumper", "illusion", "optima", "pandora"};
		...
		try {
			...
			for (String id : botnet_ids) {
				...
				for (int count = 0; count <= total_count / increment; count++) {
					...
					String template = "select count(distinct ip) as 'number' from bots where botnet_id='{0}' and timestamp<='{1}' and timestamp>'{2}'";
				...
				proxy.eval("run bot_active_vline.m;");
